# coding: utf-8
import pytest
from jinja2 import PackageLoader, Environment, Undefined

from jinja2schema.config import Config
from jinja2schema.core import infer, infer_from_ast
...
def env():
    ...
    env = Environment(loader=loader,  extensions=['jinja2.ext.i18n', 'jinja2.ext.loopcontrols'])
...
def test_trans(env, config):
    template = infer(env.loader.get_source(env, 'trans.html')[0], config)
    struct = infer_from_ast(template, config)
